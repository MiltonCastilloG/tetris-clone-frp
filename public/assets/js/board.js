...
const drawHoldTetromino = holdTetromino => {
    ...
    ctx.fillStyle = "#000";
    ctx.fillRect(0, 0, width, height);
    if (!holdTetromino || !holdTetromino.tetromino || !holdTetromino.tetromino[0]) return;

    const previewBlocks = holdTetromino.tetromino[0];
    if (!previewBlocks.length) return;

    const logicalCellSize = Math.floor(Math.min(width / HOLD_GRID_COLUMNS, height / HOLD_GRID_ROWS));
    ...
    ctx.save();
    ctx.beginPath();
    ctx.rect(gridOffsetX, gridOffsetY, renderedLogicalWidth, renderedLogicalHeight);
    ctx.clip();
...
const addUpcomingTetrominoesToBoard = tetrominoesBank => {
    ...
    const logicalCellSize = Math.floor(Math.min(
    ...
    for (let slotIndex = 0; slotIndex < maxVisible; slotIndex += 1) {
        ...
        if (!tetrominoData || !tetrominoData.tetromino || !tetrominoData.tetromino[0]) continue;
        ...
        const renderedLogicalHeight = logicalCellSize * (UPCOMMING_QUEUE_ROWS / UPCOMMING_QUEUE_SLOTS);
        const slotOffsetX = Math.floor((slotWidth - renderedLogicalWidth) / 2);
        const slotOffsetY = Math.floor(slotTop + (slotHeight - renderedLogicalHeight) / 2);
        const previewBlocks = tetrominoData.tetromino[0];
        const xs = previewBlocks.map(block => block.x);
        const ys = previewBlocks.map(block => block.y);
        const minX = Math.min(...xs);
        ...
        const pieceOffsetY = Math.floor(((UPCOMMING_QUEUE_ROWS / UPCOMMING_QUEUE_SLOTS) - pieceHeight) / 2);
        ...
        previewBlocks.forEach(({ x, y }) => {
            ...
            if (pixelY >= slotTop && pixelY + logicalCellSize <= slotBottom) {
                drawFlatPreviewCell(ctx, pixelX, pixelY, logicalCellSize, colorFactory(tetrominoData.color));
            }
        });
