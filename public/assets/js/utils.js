...
const checkBlocksCollision = (blocks, binaryMap) => blocks
    .map(({ y, x }) => y >= 0 && y < binaryMap.length
        && x >= 0
...
const checkTetrominoCollisionBottom = getBinaryMap => tetrominoState =>
    checkBlocksCollision(projectTetrominoMovement(tetrominoState, { dy: 1 }), getBinaryMap());
